...
Both of the schedulers-*.nix files set CPU and IO schedulers tuned for optimal interactive performance. schedulers-workstation.nix uses the bpfland CPU scheduler. schedulers-gaming.nix uses the lavd CPU scheduler. Use one file or the other, not both.
